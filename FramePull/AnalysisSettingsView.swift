        ...
        Button {
            ...
            HStack(spacing: 8) {
                ...
                Image(systemName: isOn.wrappedValue ? "checkmark.circle.fill" : "circle")
                    .font(.system(size: 18))
                    .foregroundColor(isOn.wrappedValue ? .framePullBlue : .secondary.opacity(0.5))
            }
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(isOn.wrappedValue ? Color.framePullLightBlue : Color.clear)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .strokeBorder(isOn.wrappedValue ? Color.framePullBlue : Color.gray.opacity(0.3), lineWidth: 1)
            )
